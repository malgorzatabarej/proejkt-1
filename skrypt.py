        ...
        for X, Y, Z in zip(X,Y,Z):
            ...
            while True:
                ...
                if abs(fp-f)<(0.000001/206265):
                    break
            l=np.arctan2(Y,X)
            transformed.append([np.rad2deg(f), np.rad2deg(l), h])
        return transformed   
    """
   Transformacja współrzędnych elipsoidalnych fi, lambda, h na współrzędne XYZ"""
    ...
    def plik(self, pliczek, transf):
        ...
        if transf == 'XYZ2FLH':
            ...
            np.savetxt(f"wynik_{transf}_{args.elip}.txt", transformed, delimiter=' ', fmt='%0.10f %0.10f %0.3f')
        elif transf == 'FLH2XYZ':
            print('jestes tutaj')
            ...
            np.savetxt(f"wynik_{transf}_{args.elip}.txt", transformed, delimiter=' ', fmt='%0.3f %0.3f')
        elif transf == 'GK1992':
